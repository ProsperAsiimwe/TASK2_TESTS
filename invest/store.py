...
class Store:
    ...
    def __init__(self, main_data, companies, companies_jcsev, companies_jgind, margin_of_safety,
        ...
        self.df_main = main_data.copy()  # Create a copy to avoid SettingWithCopyWarning
        ...
        self.calculate_missing_ratios()
        self.create_derived_variables()

        self.column_names = ["company_name", "negative_earnings", "negative_shareholders_equity", "beta_classify",
                             "acceptable_stock", "current_PE_relative_share_market_to_historical",
                             "current_PE_relative_share_sector_to_historical", "forward_PE_current_to_historical", 
                             "roe_vs_coe", "growth_cagr_vs_inflation", "relative_debt_to_equity", "systematic_risk"]
        self.df_shares = pd.DataFrame(columns=self.column_names)
        self.process()

    def calculate_missing_ratios(self):
        if 'COE' not in self.df_main.columns:
            self.df_main['COE'] = self.df_main.apply(lambda row: 
                row['RiskFreeRateOfReturn'] + row['ShareBeta'] * (row['MarketRateOfReturn'] - row['RiskFreeRateOfReturn']), 
                axis=1)

    def create_derived_variables(self):
        self.df_main['ROEvsCOE'] = self.df_main.apply(lambda row: 
            'Above' if row['ROE'] > row['COE'] else 'Below' if row['ROE'] < row['COE'] else 'EqualTo', axis=1)
        self.df_main['RelDE'] = self.df_main['Debt/Equity'] / self.df_main['Debt/EquityIndustry']
        self.df_main['CAGRvsInflation'] = self.df_main.groupby('Name')['EPS'].pct_change() - self.df_main['InflationRate']
        self.df_main['PE/PEMarket'] = self.df_main['PE'] / self.df_main['PEMarket']
        self.df_main['PE/PESector'] = self.df_main['PE'] / self.df_main['PESector']
        self.df_main['PE/HistoricalPE'] = self.df_main.groupby('Name')['PE'].transform(lambda x: x / x.mean())
    ...
    def process(self):
        ...
        for company in self.companies:
            ...
            # Skip calculations if we don't have enough data, but still add the company to df_shares
            if len(eps_year_list) < 2 or df_current_year is None or current_price is None:
                company_row = {
                    "company_name": company,
                    "negative_earnings": True,
                    "negative_shareholders_equity": True,
                    "beta_classify": False,
                    "acceptable_stock": False
                }
    ...
    def get_acceptable_stock(self, company):
        ...
        if company_data.empty:
            # If the company is not in df_shares, consider it not acceptable
            return False
