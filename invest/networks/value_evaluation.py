...
def create_value_bn():
    ve_model = gum.BayesNet('ValueEvaluation')
    
    # Add nodes
    ve_model.add(gum.LabelizedVariable('FutureSharePerformance', '', ['Low', 'Medium', 'High']))
    ve_model.add(gum.LabelizedVariable('PERelative_ShareMarket', '', ['Low', 'Medium', 'High']))
    ve_model.add(gum.LabelizedVariable('PERelative_ShareSector', '', ['Low', 'Medium', 'High']))
    ve_model.add(gum.LabelizedVariable('ForwardPE_CurrentVsHistory', '', ['Low', 'Medium', 'High']))
    ve_model.add(gum.LabelizedVariable('ValueRelativeToPrice', '', ['Low', 'Medium', 'High']))

    # Add arcs
    ve_model.addArc('FutureSharePerformance', 'PERelative_ShareMarket')
    ve_model.addArc('FutureSharePerformance', 'PERelative_ShareSector')
    ve_model.addArc('FutureSharePerformance', 'ForwardPE_CurrentVsHistory')
    ve_model.addArc('PERelative_ShareMarket', 'ValueRelativeToPrice')
    ve_model.addArc('PERelative_ShareSector', 'ValueRelativeToPrice')
    ve_model.addArc('ForwardPE_CurrentVsHistory', 'ValueRelativeToPrice')

    return ve_model

def learn_value_cpts(bn, data, method='MLE'):
...
def value_network(pe_relative_market_state, pe_relative_sector_state, forward_pe_current_vs_history_state, future_performance_state=None, learned_bn=None):
     if learned_bn is None:
        ...
        ie.setEvidence({'PERelative_ShareMarket': pe_relative_market_state,
                        'PERelative_ShareSector': pe_relative_sector_state,
                        'ForwardPE_CurrentVsHistory': forward_pe_current_vs_history_state})
        
        if future_performance_state:
            ie.setEvidence({'FutureSharePerformance': future_performance_state})
        
        ie.makeInference()
        
        value_probs = ie.posterior('ValueRelativeToPrice')
        decision_index = np.argmax(value_probs)
        decision = ['Cheap', 'FairValue', 'Expensive'][decision_index]
