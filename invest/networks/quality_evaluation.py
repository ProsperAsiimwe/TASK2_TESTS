...
def create_quality_bn():
    qe_model = gum.BayesNet('QualityEvaluation')
    
    # Add nodes
    qe_model.add(gum.LabelizedVariable('FutureSharePerformance', '', ['Low', 'Medium', 'High']))
    qe_model.add(gum.LabelizedVariable('CAGRvsInflation', '', ['Low', 'Medium', 'High']))
    qe_model.add(gum.LabelizedVariable('ROEvsCOE', '', ['Low', 'Medium', 'High']))
    qe_model.add(gum.LabelizedVariable('RelDE', '', ['Low', 'Medium', 'High']))
    qe_model.add(gum.LabelizedVariable('Quality', '', ['Low', 'Medium', 'High']))

    # Add arcs
    qe_model.addArc('FutureSharePerformance', 'CAGRvsInflation')
    qe_model.addArc('FutureSharePerformance', 'ROEvsCOE')
    qe_model.addArc('FutureSharePerformance', 'RelDE')
    qe_model.addArc('CAGRvsInflation', 'Quality')
    qe_model.addArc('ROEvsCOE', 'Quality')
    qe_model.addArc('RelDE', 'Quality')

    return qe_model

def learn_quality_cpts(bn, data, method='MLE'):
...
def quality_network(roe_vs_coe_state, relative_debt_equity_state, cagr_vs_inflation_state, systematic_risk_state=None, extension=False, learned_bn=None):
    if learned_bn is None:
        ...
        ie.setEvidence({'ROEvsCOE': roe_vs_coe_state,
                        'RelDE': relative_debt_equity_state,
                        'CAGRvsInflation': cagr_vs_inflation_state})
        
        ie.makeInference()
        
        quality_probs = ie.posterior('Quality')
        decision_index = np.argmax(quality_probs)
        decision = ['High', 'Medium', 'Low'][decision_index]
